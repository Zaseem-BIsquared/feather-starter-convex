...
import { CheckoutPage, BillingSettings } from "./index";
import { Route as CheckoutRoute } from "@/routes/_app/_auth/dashboard/_layout.checkout";
import { Route as BillingRoute } from "@/routes/_app/_auth/dashboard/_layout.settings.billing";

// -- CheckoutPage tests --

describe("CheckoutRoute.beforeLoad", () => {
  it("returns the correct title", () => {
    const context = CheckoutRoute.options.beforeLoad!({} as any);
...
test("checkout renders nothing when unauthenticated", async ({ testClient }) => {
...
test("selects plan via keyboard Enter", async ({ client, testClient, userId }) => {
  ...
  const proCard = screen.getByText("Pro plan").closest('[role="button"]')! as HTMLElement;
  proCard.focus();
...
test("billing renders nothing when unauthenticated", async ({ testClient }) => {
  await seedPlans(testClient);
